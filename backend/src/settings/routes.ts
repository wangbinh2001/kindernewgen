...
export const schoolInfoValueSchema = valueSchema.superRefine(
  (value, context) => {
    addAddressPairIssue(value, context, "province", "commune");
  },
);
...
settingsRoutes.put("/:key", async (context) => {
  ...
  const value = (
    key.success && key.data === "school-info"
      ? schoolInfoValueSchema
      : valueSchema
