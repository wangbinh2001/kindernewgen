...
calculationRoutes.get(
  ...
  async (context) => {
    ...
    if (!parsed.success)
      return validationError(context, "Invalid history query");
    const schoolId = schoolIdOf(context);
    const { month, studentId, page, limit } = parsed.data;
    try {
      const data = await withTenant(schoolId, async (tx) => {
        const conditions = [eq(tuitionHistory.schoolId, schoolId)];
        if (month) conditions.push(eq(tuitionHistory.month, month));
        if (studentId) conditions.push(eq(tuitionHistory.studentId, studentId));
        ...
        const items = await tx
          ...
          .limit(limit)
          .offset((page - 1) * limit);
        return { items, total: Number(total?.value ?? 0), page, limit };
      });
    ...
    try {
      const data = await withTenant(schoolId, async (tx) => {
        const [history] = await tx
          .select({
            id: tuitionHistory.id,
            schoolId: tuitionHistory.schoolId,
            studentId: tuitionHistory.studentId,
            studentName: students.fullName,
            month: tuitionHistory.month,
      ...
      return data
        ...
        : notFoundError(context, "Tuition history not found");
    } catch {
      return internalError(context);
    }
  },
      ...
      const result = await withTenant(schoolId, async (tx) => {
        ...
        for (const { record, opening } of prepared) {
          ...
          await notifyParentsOfStudents(tx, schoolId, [record.studentId], {
            ...
            message: `Học phí tháng ${parsed.data.month}: ${record.finalAmount}`,
            entityType: "tuition_history",
            entityId: history.id,
            data: { month: parsed.data.month, finalAmount: record.finalAmount },
          });
