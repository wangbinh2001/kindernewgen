      ...
      const result = await withTenant(schoolId, async (tx) => {
        ...
        for (const { record, scope, existing } of prepared) {
          ...
          if (record.state === "confirmed") {
            await notifyParentsOfStudents(tx, schoolId, [record.studentId], {
              type: "attendance",
              title: "Cập nhật điểm danh",
              message: `Điểm danh: ${record.status}`,
              entityType: "attendance",
              entityId: row.id,
              data: { date: record.date, status: record.status },
            });
          }
          await recordAuditLog(tx, {
            ...
            before: existing,
            after: row,
            extra: { optionalFeeIds: record.optionalFeeIds },
...
const qrScanSchema = z
  .object({
    studentId: z.string().min(1),
    type: z.enum(["check_in", "check_out"]),
    // Chỉ dùng trong integration test; production luôn lấy giờ server.
    timestamp: z
      .string()
      .regex(/^\d{2}:\d{2}$/)
      .optional(),
...
function getVietnamNow(): { date: string; time: string } {
  const now = new Date();
...
function settingCheckoutEnd(value: unknown): string {
  if (!value || typeof value !== "object") return "17:00";
  const candidate = (value as Record<string, unknown>).checkoutEnd;
  return typeof candidate === "string" && parseTimeToMinutes(candidate) !== null
    ? candidate
    : "17:00";
}
...
attendanceRoutes.post(
  ...
  requireRole("school_admin", "teacher"),
  async (context) => {
    const parsed = qrScanSchema.safeParse(await readJson(context));
    if (!parsed.success) {
      return validationError(context, "Invalid QR scan data");
    }
    const schoolId = schoolIdOf(context);
    const claims = claimsOf(context);
    ...
    const scanDate = parsed.data.date ?? vnNow.date;
    const scanTime = parsed.data.timestamp ?? vnNow.time;
    ...
    try {
      const result = await withTenant(schoolId, async (tx) => {
        ...
        // Nếu là giáo viên, xác thực phân công lớp
        if (isTeacher(claims)) {
          const [assignment] = await tx
            .select({ id: teacherAssignments.id })
            .from(teacherAssignments)
        ...
        // Tìm bản ghi điểm danh hôm nay nếu có
        const [existing] = await tx
          .select()
          .from(attendance)
          .where(
            and(
              ...
              eq(attendance.date, scanDate),
              ne(attendance.state, "voided"),
            ),
          );

        // Lấy cấu hình giờ tan trường (mặc định 17:00)
        let checkoutEnd = "17:00";
        const [settingRow] = await tx
          ...
          .where(
            and(
              ...
              eq(schoolSettings.settingKey, "attendance-config"),
            ),
          );
        if (
          settingRow?.settingValue &&
          typeof settingRow.settingValue === "object" &&
          "checkoutEnd" in settingRow.settingValue &&
          typeof (settingRow.settingValue as Record<string, unknown>)
            .checkoutEnd === "string"
        ) {
          checkoutEnd = (settingRow.settingValue as { checkoutEnd: string })
            .checkoutEnd;
        }
        ...
        if (type === "check_in") {
          if (existing) {
            ...
            // Update checkInTime nếu chưa có
            const [updated] = await tx
              .update(attendance)
              .set({
                status: "present",
                checkInTime: existing.checkInTime ?? scanTime,
                updatedAt: now,
                updatedBy: claims.sub,
              })
              .where(eq(attendance.id, existing.id))
              .returning();
            if (!updated) throw new Error("Attendance was not updated");
            await recordAuditLog(tx, {
              actorId: claims.sub,
              schoolId,
              action: "qr_check_in",
          ...
          const [created] = await tx
            ...
            .returning();
          if (!created) throw new Error("Attendance was not created");
          await recordAuditLog(tx, {
            actorId: claims.sub,
            schoolId,
            action: "qr_check_in",
          ...
          const endMinutes = parseTimeToMinutes(checkoutEnd);
          const diffMinutes =
            scanMinutes !== null && endMinutes !== null
              ? Math.max(0, scanMinutes - endMinutes)
              : 0;

          let overtimeStart: string | null = null;
          let overtimeEnd: string | null = null;
          let overtimeHours: string | null = null;

          if (diffMinutes > 0) {
            overtimeStart = checkoutEnd;
            overtimeEnd = scanTime;
            overtimeHours = (diffMinutes / 60).toFixed(2);
          }
          ...
          if (existing) {
            const [updated] = await tx
              ...
              .set({
                ...
                overtimeStart: overtimeStart ?? existing.overtimeStart,
                overtimeEnd: overtimeEnd ?? existing.overtimeEnd,
          ...
          // Chưa từng check-in mà phụ huynh tới đón -> tạo bản ghi với status = present
          const [created] = await tx
            .insert(attendance)
            .values({
              id: crypto.randomUUID(),
    ...
    try {
      const result = await withTenant(schoolId, async (tx) => {
        ...
        await recordAuditLog(tx, {
          ...
          after: correction,
          extra: {
            optionalFeeIds: parsed.data.optionalFeeIds,
