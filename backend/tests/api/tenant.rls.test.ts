...
test("tenant context isolates tenant rows (students)", async () => {
  ...
  await db.execute(
    sql`INSERT INTO schools (id, name) VALUES (${schoolA}, ${`School A ${schoolA}`}), (${schoolB}, ${`School B ${schoolB}`})`,
  );

  // Insert students in their respective tenant contexts
  await withTenant(schoolA, async (tx) => {
    await tx.execute(
      sql`INSERT INTO students (id, school_id, full_name) VALUES (${studentA}, ${schoolA}, 'Student A')`,
    );
  });

  await withTenant(schoolB, async (tx) => {
    await tx.execute(
      sql`INSERT INTO students (id, school_id, full_name) VALUES (${studentB}, ${schoolB}, 'Student B')`,
    );
  });
  ...
  const countA = await withTenant(schoolA, async (tx) => {
    const result = await tx.execute(
      sql`SELECT id FROM students WHERE id IN (${studentA}, ${studentB})`,
    );
    return result.count;
  });
